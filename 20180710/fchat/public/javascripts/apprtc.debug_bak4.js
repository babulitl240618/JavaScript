...
var enums = {"EventType":{"ICE_CONNECTION_STATE_CONNECTED":3, "ROOM_SIZE_2":4}, "RequestField":{"MessageType":{"EVENT":"event"}, "CLIENT_TYPE":"client_type", "EventField":{"EVENT_TIME_MS":"event_time_ms", "ROOM_ID":"room_id", "EVENT_TYPE":"event_type", "FLOW_ID":"flow_id"}, "TYPE":"type", "EVENT":"event", "REQUEST_TIME_MS":"request_time_ms"}, "ClientType":{"UNKNOWN":0, "ANDROID":4, "DESKTOP":2, "IOS":3, "JS":1}};
var remoteVideo = $("#remote-video");
var streamCounter = 0;
var UI_CONSTANTS = {confirmJoinButton:"#confirm-join-button", confirmJoinDiv:"#confirm-join-div", confirmJoinRoomSpan:"#confirm-join-room-span", fullscreenSvg:"#fullscreen", hangupSvg:"#hangup", icons:"#icons", infoDiv:"#info-div", localVideo:"#local-video", miniVideo:"#mini-video", con1Video:"#con1", con2Video:"#con2", muteAudioSvg:"#mute-audio", muteVideoSvg:"#mute-video", newRoomButton:"#new-room-button", newRoomLink:"#new-room-link", privacyLinks:"#privacy", remoteVideo:"#remote-video", rejoinButton:"#rejoin-button", 
rejoinDiv:"#rejoin-div", rejoinLink:"#rejoin-link", roomLinkHref:"#room-link-href", roomSelectionDiv:"#room-selection", roomSelectionInput:"#room-id-input", roomSelectionInputLabel:"#room-id-input-label", roomSelectionJoinButton:"#join-button", roomSelectionRandomButton:"#random-button", roomSelectionRecentList:"#recent-rooms-list", sharingDiv:"#sharing-div", statusDiv:"#status-div", videosDiv:"#videos"};
var AppController = function(loadingParams) {
  ...
  this.con1Video_ = $(UI_CONSTANTS.con1Video);
  this.con2Video_ = $(UI_CONSTANTS.con2Video);
    ...
    if(parseInt(is_busy)==1){
      ...
      if(user_id==partner_id){
        ...
        var hangup_id=0;
          ...
          window.location.href = '/hayven/chat/' + hangup_id + '/' + encodeURI(hangup_name) + '/' + encodeURI(hangup_img);
...
AppController.prototype.finishCallSetup_ = function(roomId) {
  ...
  if (!isChromeApp()) {
    window.onbeforeunload = function() {
      setCookie('call_status','reload',1);
...
AppController.prototype.hangup_ = function() {
  ...
  // var el = document.querySelector('#hangup');
  ...
  socket.emit('call_hangup', { hangup_id : hangup_id , partner_id : partner_id });

  socket.emit('userbusy_dbup', { user_id : user_id , is_busy : 0 } , function (data) {
    if(data){
...
AppController.prototype.onRemoteStreamAdded_ = function(stream) {
  ...
  if (streamCounter == 2) {
    this.con1Video_.srcObject = stream;
  } else {
    if (streamCounter == 3) {
      this.con2Video_.srcObject = stream;
    } else {
      if (streamCounter = 1) {
...
AppController.prototype.onLocalStreamAdded_ = function(stream) {
  ...
  if(call_type=='video'){
    document.getElementById("call_userimg").style.display = "none";  
  }
...
AppController.prototype.transitionToActive_ = function() {
  ...
  this.displayStatus_("");
   ...
   var aud_tone = document.getElementById("mdCallTone");
   aud_tone.pause();
   aud_tone.currentTime = 0;

   document.getElementById("call_loading").style.display = "none";
...
AppController.prototype.toggleAudioMute_ = function() {
  ...
  var logo = document.getElementById('mute-audio');
  
  if(/\bon/.test(document.querySelector('#mute-audio').className)){
    
    logo.src = "/images/call/mute-on_56px.png";
  }else{
    logo.src = "/images/call/mute-off_56px.png";
  }
...
AppController.prototype.toggleVideoMute_ = function() {
  ...
  var logo = document.getElementById('mute-video');
  
  if(/\bon/.test(document.querySelector('#mute-video').className)){
    
    logo.src = "/images/call/video-on_56px.svg";
  }else{
    logo.src = "/images/call/video-off_56px.svg";
  }
  //console.log(document.querySelector('#mute-video').classList); 
};
...
Call.prototype.maybeGetIceServers_ = function() {
  ...
  if (shouldRequestIceServers) {
    ...
    iceServerPromise = requestIceServers(requestUrl, this.params_.iceServerTransports).then(function(iceServers) {
      ...
      console.log("iccccccccccccccccccccc:::",servers);
...
PeerConnectionClient.prototype.startAsCaller = function(offerOptions) {
  ...
  var room_genid = document.getElementById("room-id-input").value;

  if(call_type=='audio'){

    socket.emit('audiocall_req', {
      my_name: user_fullname, my_id : user_id, my_img: user_img,
      to_name: partner_name, to_id : to, to_img: partner_img, room_genid : room_genid 
    });
    ...
    socket.emit('videocall_req', {
      my_name: user_fullname, my_id : user_id, my_img: user_img,
      to_name: partner_name, to_id : partner_id , to_img: partner_img, room_genid : room_genid 
    });
...
PeerConnectionClient.prototype.setLocalSdpAndNotify_ = function(sessionDescription) {
  ...
  if (this.onsignalingmessage) {
    ...
    socket.emit('userbusy_dbup', { user_id : user_id , is_busy : 1 } , function (data) {
      if(data){
        socket.emit('userbusy_dbup', { user_id : partner_id , is_busy : 1 } , function (data) {
          //alert('signaling done');  
        });
      }  
    });
  }
