  ...
  Promise.resolve(paramsPromise).then(function(newParams) {
    ...
    if(parseInt(is_busy)==1){
      ...
      if(user_id==partner_id){
        ...
        window.location.href='/hayven/chat/'+'personal'+'/'+ hangup_id +'/'+ encodeURI(conversation_id) +'/'+ encodeURI(hangup_name) +'/' + encodeURI(hangup_img);
...
AppController.prototype.hangup_ = function() {
  ...
  if(user_id == caller_id){
    ...
    var call_status = document.getElementById('call_status').innerHTML;
    
    if(call_status=='CALLING'){

      socket.emit('call_hangup_before', {
        hangup_id : hangup_id , 
